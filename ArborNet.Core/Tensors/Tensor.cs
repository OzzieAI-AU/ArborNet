...
namespace ArborNet.Core.Tensors
{
    ...
    public sealed class Tensor : ITensor
    {
        ...
        // =================================================================================
        // STATIC INITIALIZATION FACTORIES WITH SAFE DEVICE DEGRADATION
        // =================================================================================

        private static Device ResolveDevice(Device? device)
        {
            device ??= Device.CPU;
            if (device.Type == DeviceType.CUDA && !CUDA.IsAvailable())
            {
                return Device.CPU;
            }
            return device;
        ...
        public static ITensor Zeros(TensorShape shape, Device? device = null)
        {
            ...
            ITensor backend = resolved.Type == DeviceType.CUDA
                ? CudaBackend.Zeros(shape, resolved)
                : CpuBackend.Zeros(shape, resolved);
        ...
        public static ITensor Ones(TensorShape shape, Device? device = null)
        {
            ...
            var resolved = ResolveDevice(device);
            ITensor backend = resolved.Type == DeviceType.CUDA
                ? CudaBackend.Ones(shape, resolved)
                : CpuBackend.Ones(shape, resolved);
            return new Tensor(backend);
        }
        ...
        public static ITensor FromScalar(float value, Device? device = null)
        {
            var resolved = ResolveDevice(device);
            ITensor backend = resolved.Type == DeviceType.CUDA
                ? CudaBackend.FromScalar(value, resolved)
                : CpuBackend.FromScalar(value, resolved);
            return new Tensor(backend);
        }
        ...
        public static ITensor FromArray(float[] data, TensorShape shape, Device? device = null)
        {
            ...
            var resolved = ResolveDevice(device);
            ITensor backend = resolved.Type == DeviceType.CUDA
                ? CudaBackend.FromArray(data, shape, resolved)
                : CpuBackend.FromArray(data, shape, resolved);
            return new Tensor(backend);
        }
        ...
        public static ITensor Rand(TensorShape shape, Device? device = null)
        {
            ...
            var resolved = ResolveDevice(device);
            ITensor backend = resolved.Type == DeviceType.CUDA
                ? CudaBackend.Rand(shape, resolved)
                : CpuBackend.Rand(shape, resolved);
            return new Tensor(backend);
        }
        ...
        public static ITensor Randn(TensorShape shape, Device? device = null)
        {
            ...
            var resolved = ResolveDevice(device);
            ITensor backend = resolved.Type == DeviceType.CUDA
                ? CudaBackend.Randn(shape, resolved)
                : CpuBackend.Randn(shape, resolved);
            return new Tensor(backend);
        }
        ...
        public static ITensor Full(TensorShape shape, float value, Device? device = null)
        {
            if (shape == null) throw new ArgumentNullException(nameof(shape));
            var resolved = ResolveDevice(device);
            return Zeros(shape, resolved).Add(FromScalar(value, resolved));
        }
        ...
        public static ITensor Eye(int n, Device? device = null)
        {
            ...
            var resolved = ResolveDevice(device);
            ITensor backend = resolved.Type == DeviceType.CUDA
                ? CudaBackend.Eye(n, resolved)
                : CpuBackend.Eye(n, resolved);
            return new Tensor(backend);
        }
        ...
        public ITensor Concat(IEnumerable<ITensor> others, int axis = 0) => new Tensor(_backend.Concat(others.Select(Unwrap), axis));
